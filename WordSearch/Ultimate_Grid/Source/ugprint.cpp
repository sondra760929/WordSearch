						...
						int endCol,long endRow)
{
	...
	float ratio = (float)virtualWidthPX / (float)width;
	width = 0;
	m_printVScale *= ratio;
	m_printHScale *= ratio;
	//save the margins
	...
	{
		m_virPixWidth = (int)paperWidthPX / 2000;  //300
	...
	for(row = startRow;row <=endRow;row++)
	{
		...
		height += h;
		if(height > virtualHeightPX / ratio)
		{
			m_pagesHigh ++;
			if(m_printTopHeading)
				height = m_ctrl->GetTH_Height();
